...
if __name__ == "__main__":
    print("=" * 60)
    print("Testing MongoDB Review Integration")
    print("=" * 60)

    mongo_id = submit_review(
        customer_id=1,
        restaurant_id=1,
        reservation_id=1,
        rating=4.5,
        title="Excellent evening at Repub Lounge!",
        body=(
            "We had a wonderful time at Repub Lounge last Friday. "
            "The service was attentive without being intrusive, and the "
            "grilled tilapia was cooked to perfection. The atmosphere was "
            "lively but not too loud. Only minor issue was a 10-minute wait "
            "for our starters, but the quality made up for it. Will definitely "
            "return for special occasions."
        ),
        tags=['great_food', 'attentive_service', 'good_ambiance', 'romantic'],
        food_rating=4.8,
        service_rating=4.5,
        ambiance_rating=4.3
    )
    print(f"\nReview stored: {mongo_id}")

    full_review = get_review_full(mongo_id)
    print(f"\nFull review title: {full_review['title']}")
    print(f"Tags: {full_review['tags']}")
    print(f"Food rating: {full_review['food_rating']}")

    add_owner_response(
        mongo_id,
        "Thank you so much for the kind words! We look forward to welcoming "
        "you back. Apologies for the wait — we'll work on that!"
    )

    reviews = get_restaurant_reviews(restaurant_id=1, limit=5)
    print(f"\nReviews for restaurant 1: {len(reviews)}")

    results = search_reviews(restaurant_id=1, keyword="tilapia")
    print(f"Search 'tilapia': {len(results)} results")

    stats = get_review_stats(restaurant_id=1)
    print(f"\nReview stats: {stats}")
